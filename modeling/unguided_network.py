...
def build_depth_backbone(pretrained=True):
    model = CNN()
    if pretrained:
        checkpoint_dict = torch.load('/home/ash/Small-Obs-Project/nconv/workspace/exp_guided_enc_dec/unguided_network_pretrained/CNN_ep0005.pth.tar')
...
if __name__=="__main__":
    ...
    # writer = SummaryWriter("/home/ash/Small-Obs-Project/nconv/logs/")

    model = CNN()
    model = model.cuda()
    depth_img_path = "/scratch/ash/iiit_data/train/seq_1/depth/"
    depth_files = sorted(os.listdir(depth_img_path))

    inp = Image.open(os.path.join(depth_img_path,depth_files[300]))
    inp = np.array(inp)
    inp = inp/256.
    ...
    input_d = torch.Tensor(inp).cuda()
    input_c = torch.Tensor(conf).cuda()
    start = time.time()
    with torch.no_grad():
        depth,out_c = model.forward(input_d,input_c)
    print("Inference took:{} secs".format(time.time() - start))

    pred = depth.detach().cpu().numpy()
    print(pred.shape)
    pred = pred.squeeze()
    confidence = out_c.detach().cpu().numpy()
    confidence = confidence.squeeze()
    # print(summary(model, [(1,720, 1280),(1,720,1280)],batch_size=1))
    # writer.add_graph(model,(input_d,input_c),True)
